...
function CheckoutSuccessPageContent() {
  ...
  const orderNumber = searchParams.get('orderId') || searchParams.get('order') || '';
        ...
        <div className="flex flex-col gap-2 pt-4">
          {orderNumber && (
            <Link
              ...
              className="bg-[var(--brand-primary)] text-white text-sm font-bold py-3 rounded-xl"
            >
              Track Order
            </Link>
          )}
          ...
          <Link href="/" className="text-xs font-semibold text-[var(--brand-primary)]">
            Continue Shopping
          </Link>
        </div>
      </main>
