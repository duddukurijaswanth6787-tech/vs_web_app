  ...
  if (!isInitializing && !isAuthenticated) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <Link href={`/login?redirect=/orders/review/${orderNumber}`} className="text-sm font-bold text-[var(--brand-primary)]">
          Login to review
        </Link>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[var(--page-bg)] flex flex-col font-sans antialiased text-neutral-900">
      <header className="sticky top-0 z-50 bg-white border-b border-neutral-100 px-4 py-3 flex items-center gap-3">
        ...
        <h1 className="text-lg font-bold font-serif text-[var(--brand-primary)]">Review Order</h1>
      </header>
