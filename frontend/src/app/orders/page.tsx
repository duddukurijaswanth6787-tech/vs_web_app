  ...
  if (!isInitializing && !isAuthenticated) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-3 px-4">
        ...
        <Link href="/login?redirect=/orders" className="bg-[var(--brand-primary)] text-white text-xs font-bold px-5 py-2.5 rounded-xl">
          Login
        </Link>
      </div>
    );
        ...
        {orders.map((order: OrderDto) => (
          <div key={order.id || order.orderNumber} className="bg-white border border-neutral-200 rounded-2xl p-4 space-y-2">
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="text-[11px] font-bold uppercase bg-sky-50 text-[var(--brand-primary)] px-2 py-1 rounded-lg">
                {order.status}
              </span>
            </div>
            ...
            <div className="flex flex-wrap gap-2 text-xs font-semibold">
              <Link href={`/orders/details/${order.orderNumber}`} className="text-[var(--brand-primary)] font-bold">
                View Details
              </Link>
