      ...
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl border border-neutral-200 max-h-[95vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mt-4 space-y-4 text-xs">
          ...
          <div className="grid grid-cols-2 gap-3 pt-2 border-t border-neutral-100">
            ...
            <div><label className="block text-[10px] font-bold text-neutral-500 uppercase">{selectedType === CouponType.PERCENTAGE ? 'Rate (%)' : selectedType === CouponType.FLAT ? 'Amount (₹)' : 'Value'}</label><input type="number" min="0" onKeyDown={preventNegativeKeys} disabled={selectedType === CouponType.FREE_SHIPPING} {...register('value', { valueAsNumber: true })} className="mt-1 w-full bg-neutral-50 border border-neutral-200 rounded-lg px-2.5 py-1.5 disabled:opacity-50" />{errors.value && <p className="text-[9px] text-red-650 mt-1">{errors.value.message}</p>}</div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="block text-[10px] font-bold text-neutral-500 uppercase">Min Order (₹)</label><input type="number" min="0" onKeyDown={preventNegativeKeys} {...register('minOrderAmount', { valueAsNumber: true })} className="mt-1 w-full bg-neutral-50 border border-neutral-200 rounded-lg px-2.5 py-1.5" /></div>
            <div><label className="block text-[10px] font-bold text-neutral-500 uppercase">Max Cap (₹)</label><input type="number" min="0" onKeyDown={preventNegativeKeys} disabled={selectedType !== CouponType.PERCENTAGE} {...register('maxDiscountAmount', { valueAsNumber: true })} className="mt-1 w-full bg-neutral-50 border border-neutral-200 rounded-lg px-2.5 py-1.5 disabled:opacity-50" /></div>
          </div>
          <div className="grid grid-cols-2 gap-3 pt-2 border-t border-neutral-100">
            <div><label className="block text-[10px] font-bold text-neutral-500 uppercase">Total Usage Limit</label><input type="number" min="0" onKeyDown={preventNegativeKeys} {...register('usageLimit', { valueAsNumber: true })} className="mt-1 w-full bg-neutral-50 border border-neutral-200 rounded-lg px-2.5 py-1.5" /></div>
            <div><label className="block text-[10px] font-bold text-neutral-500 uppercase">Per Customer Limit</label><input type="number" min="1" onKeyDown={preventNegativeKeys} {...register('perCustomerLimit', { valueAsNumber: true })} className="mt-1 w-full bg-neutral-50 border border-neutral-200 rounded-lg px-2.5 py-1.5" />{errors.perCustomerLimit && <p className="text-[9px] text-red-650 mt-1">{errors.perCustomerLimit.message}</p>}</div>
          </div>
          <div className="grid grid-cols-2 gap-3 pt-2 border-t border-neutral-100">
