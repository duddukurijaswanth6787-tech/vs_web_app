  ...
  if (!isInitializing && !isAuthenticated) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <Link href="/login?redirect=/profile/addresses/add" className="text-sm font-bold text-[var(--brand-primary)]">
          Login required
        </Link>
      </div>
    );
    ...
    <div className="min-h-screen bg-[var(--page-bg)] flex flex-col font-sans antialiased text-neutral-900">
      ...
      <main className="max-w-md mx-auto w-full px-4 py-6 flex-1">
        <div className="flex items-center gap-3 mb-5">
          ...
          <h1 className="text-lg font-bold font-serif text-[var(--brand-primary)]">Add New Address</h1>
        </div>
        ...
        <form onSubmit={onSubmit} className="bg-white border border-neutral-200/90 rounded-3xl p-6 space-y-4 shadow-xs">
              ...
              {[
                ...
                <button
                  ...
                  className={`py-2 px-3 rounded-xl border text-xs font-bold flex items-center justify-center gap-1.5 transition-all ${
                    form.label === label
                      ? 'border-[var(--brand-primary)] bg-sky-50/70 text-[var(--brand-primary)]'
          ...
          <label className="flex items-center gap-2.5 pt-1 cursor-pointer">
            <input
              ...
              className="w-4 h-4 accent-[var(--brand-primary)] rounded-md"
            />
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-[var(--brand-primary)] hover:bg-[var(--brand-primary-dark)] text-white text-sm font-bold py-3.5 rounded-xl disabled:opacity-60 transition-all shadow-xs"
          >
