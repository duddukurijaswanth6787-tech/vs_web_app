  ...
  if (!isInitializing && !isAuthenticated) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <Link href="/login?redirect=/profile/notifications" className="text-sm font-bold text-[#800020]">
          Login required
        </Link>
      </div>
    );
    ...
    <div className="min-h-screen bg-[#FDFBFB] flex flex-col font-sans antialiased text-neutral-900">
      ...
      <header className="sticky top-0 z-50 bg-white border-b border-neutral-100 px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-bold font-serif text-[#800020]">Notifications</h1>
        </div>
        <button
          type="button"
          className="text-xs font-bold text-[#800020]"
          onClick={async () => {
        ...
        {items.map((n) => (
          <button
            ...
            className={`w-full text-left bg-white border rounded-2xl p-4 ${
              n.isRead ? 'border-neutral-200' : 'border-[#800020]/30 bg-rose-50/40'
            }`}
          >
