  ...
  if (!isInitializing && !isAuthenticated) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <Link href="/login?redirect=/profile/payments" className="text-sm font-bold text-[var(--brand-primary)]">
          Login required
        </Link>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[var(--page-bg)] flex flex-col font-sans antialiased text-neutral-900">
      <header className="sticky top-0 z-50 bg-white border-b border-neutral-100 px-4 py-3 flex items-center gap-3">
        ...
        <h1 className="text-lg font-bold font-serif text-[var(--brand-primary)]">Payment Methods</h1>
      </header>
        ...
        {(Array.isArray(data) ? data : []).map((mItem) => {
          ...
          return (
            <div key={String(m.code || '')} className="bg-white border border-neutral-200 rounded-2xl p-4 flex gap-3">
              <CreditCard className="w-5 h-5 text-[var(--brand-primary)] shrink-0" />
              <div>
