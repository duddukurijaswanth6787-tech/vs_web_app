  ...
  return (
    <div className="space-y-3">
      {selectedIds && selectedIds.size > 0 && bulkActions && (
        <div className="flex items-center gap-3 bg-neutral-50 rounded-xl px-4 py-2.5 border border-neutral-200">
          <span className="text-xs font-medium text-neutral-700">{selectedIds.size} selected</span>
          {bulkActions}
          <button onClick={() => onSelectionChange?.(new Set())} className="ml-auto text-xs text-neutral-500 hover:text-neutral-700">Clear</button>
        </div>
      )}
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between bg-white rounded-xl border border-neutral-200 px-4 py-3 shadow-sm">
          <p className="text-xs text-neutral-500">{total} items · Page {page} of {totalPages}</p>
          <div className="flex gap-1">
            <button disabled={page <= 1} onClick={() => onPageChange(page - 1)}
              className="px-3 py-1.5 border border-neutral-200 rounded-lg text-xs disabled:opacity-30 hover:bg-neutral-50">Prev</button>
            <button disabled={page >= totalPages} onClick={() => onPageChange(page + 1)}
              className="px-3 py-1.5 border border-neutral-200 rounded-lg text-xs disabled:opacity-30 hover:bg-neutral-50">Next</button>
          </div>
        </div>
      )}
    </div>
