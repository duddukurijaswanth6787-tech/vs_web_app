    ...
    <section className="w-full max-w-[1440px] mx-auto py-4 sm:py-4 lg:hidden">
      <div className="flex overflow-x-auto gap-3.5 px-4 sm:px-6 pb-2.5 pt-1 scrollbar-none snap-x snap-mandatory">
        {categories.map((cat) => (
          <div
            key={cat.id || cat.slug}
            className="w-[85vw] max-w-[340px] sm:w-[360px] shrink-0 snap-start bg-[#FAF3F3] rounded-3xl p-4 sm:p-5 border border-rose-100/80 shadow-2xs flex items-center justify-between relative overflow-hidden group"
          >
