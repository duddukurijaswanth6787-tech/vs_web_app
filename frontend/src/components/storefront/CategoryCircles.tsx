...
export function CategoryCircles() {
  ...
  const categories: CategoryItem[] = useMemo(() => {
    ...
    return mainCategories.map((cat) => {
      ...
      const finalUrl = hasImage ? rawImg : getCategoryFallbackImage(slug, name);
      return {
        id: String(c.id || ''),
        name,
        slug,
              ...
              >
                ...
                <div className="w-[85px] h-[85px] sm:w-[100px] sm:h-[100px] rounded-full overflow-hidden border-2 border-[#DCEBFA] p-0.5 bg-white shadow-2xs group-hover:border-[#1769D2] group-hover:shadow-md transition-all duration-300">
                  <div className="w-full h-full rounded-full overflow-hidden relative">
                    <Image
                      src={src || getCategoryFallbackImage(cat.slug, cat.name)}
                      alt={cat.name}
                      fill
                      priority={idx < 4}
                      sizes="100px"
                      unoptimized={isLocalOrPlaceholder(src || getCategoryFallbackImage(cat.slug, cat.name))}
