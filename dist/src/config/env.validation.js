...
if ((!process.env.REDIS_PORT || String(process.env.REDIS_PORT).trim() === '') &&
...
if ((process.env.REDIS_URL || process.env.REDISURL) &&
    (!process.env.REDIS_HOST || process.env.REDIS_HOST.trim() === '')) {
    try {
        const redisUrl = new URL(process.env.REDIS_URL || process.env.REDISURL || '');
...
exports.envValidationSchema = Joi.object({
    ...
    CORS_ALLOWED_HEADERS: Joi.string().default('Content-Type,Accept,Authorization,x-correlation-id,x-request-id'),
    ...
    STORAGE_ALLOWED_MIME_TYPES: Joi.string().default('image/jpeg,image/png,image/webp,image/avif,application/pdf,video/mp4'),
    ...
    AWS_S3_BUCKET: Joi.string().when('STORAGE_PROVIDER', {
        is: 's3',
        then: Joi.required().description('AWS_S3_BUCKET is required when STORAGE_PROVIDER=s3'),
        otherwise: Joi.string().allow('').default(''),
    }),
    ...
    HTTP_LOG_SUCCESS: Joi.boolean().description('Log successful, fast requests too. Defaults off in production to stay under the platform log rate cap.'),
