...
let PrinterService = class PrinterService {
    ...
    async getStoreSettings() {
        ...
        return {
            ...
            storeDescription: settings.storeDescription || FALLBACK_SETTINGS.storeDescription,
    ...
    computeTotals(dto) {
        ...
        const gstRateHalf = taxableAmount > 0
    ...
    async generateHtmlInvoiceReceipt(dto) {
        const store = await this.getStoreSettings();
        const { subtotal, discountTotal, taxableAmount, taxTotal, cgstAmount, sgstAmount, gstRateHalf, } = this.computeTotals(dto);
...
<body>
  ...
  <div class="footer-line">♥ YOUR TRUST, OUR INSPIRATION ♥</div>
    ...
    async buildEscPosInvoiceReceipt(dto) {
        const store = await this.getStoreSettings();
        const { subtotal, discountTotal, taxableAmount, taxTotal, cgstAmount, sgstAmount, gstRateHalf, } = this.computeTotals(dto);
        const commands = [];
        const appendBytes = (bytes) => commands.push(...bytes);
        ...
        if (dto.cashierName)
            appendLine(`Cashier : ${dto.cashierName}`);
        ...
        if (dto.transactionId)
            appendLine(`Txn ID  : ${dto.transactionId}`);
        appendLine('--------------------------------');
        appendBytes([0x1b, 0x61, 0x01]);
        const qrData = `https://${store.website}/contact`;
        const storeQr = (pL, pH, cn, fn, data = []) => appendBytes([0x1d, 0x28, 0x6b, pL, pH, 0x31, cn, fn, ...data]);
        storeQr(3, 0, 0x31, 0x43, [0x06]);
        storeQr(3, 0, 0x31, 0x45, [0x31]);
        ...
        appendBytes([0x1d, 0x28, 0x6b, 0x03, 0x00, 0x31, 0x51, 0x30]);
