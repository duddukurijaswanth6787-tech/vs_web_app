...
let DashboardService = class DashboardService {
    ...
    async getSummary() {
        ...
        const [totalOrders, totalCustomers, totalProducts, pendingOrders, revenueAgg, lowStockCount, recentOrders, topProducts, todayRevenueAgg, todayOrders, categoriesCount, brandsCount, activeCoupons, outOfStockProducts, pendingReviews, returnsCount, cancelledOrders, todayItemsSoldAgg,] = await Promise.all([
    ...
    async getPaymentAnalytics(dateFrom, dateTo) {
        const paymentWhere = {};
        if (dateFrom)
            paymentWhere.createdAt = { gte: new Date(dateFrom) };
        if (dateTo)
    ...
    async getSalesChart(period = 'monthly') {
        const now = new Date();
        let startDate;
        let groupBy;
        if (period === 'weekly') {
