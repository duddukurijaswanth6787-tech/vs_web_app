...
let WarehouseService = class WarehouseService {
    warehouseRepository;
    auditService;
    constructor(warehouseRepository, auditService) {
    ...
    async findById(id) {
        ...
        if (!wh || wh.deletedAt)
            throw new exceptions_1.BusinessException('Warehouse not found', 'WAREHOUSE_001');
        return wh;
    }
    async create(dto, userId) {
        const existing = await this.warehouseRepository.findByCode(dto.code);
        if (existing)
            throw new exceptions_1.BusinessException('Warehouse code already exists', 'WAREHOUSE_002');
        if (dto.isDefault)
    ...
    async restore(id, userId) {
        ...
        if (!wh)
            throw new exceptions_1.BusinessException('Warehouse not found', 'WAREHOUSE_001');
        if (!wh.deletedAt)
            throw new exceptions_1.BusinessException('Warehouse is not deleted', 'WAREHOUSE_003');
    ...
    async assignInventory(warehouseId, dto) {
        const wh = await this.warehouseRepository.findById(warehouseId);
        if (!wh || wh.deletedAt)
            throw new exceptions_1.BusinessException('Warehouse not found', 'WAREHOUSE_001');
        const existing = await this.warehouseRepository.findWarehouseInventory(warehouseId, dto.variantId);
        if (existing)
            throw new exceptions_1.BusinessException('Inventory already exists for this variant in this warehouse', 'WAREHOUSE_004');
    ...
    async updateWarehouseInventoryCount(warehouseId, variantId, dto, userId) {
        const wh = await this.warehouseRepository.findById(warehouseId);
        if (!wh || wh.deletedAt)
            throw new exceptions_1.BusinessException('Warehouse not found', 'WAREHOUSE_001');
        const existing = await this.warehouseRepository.findWarehouseInventory(warehouseId, variantId);
        if (!existing)
            throw new exceptions_1.BusinessException('No inventory record for this variant in this warehouse -- assign it first', 'WAREHOUSE_006');
        const previousQuantity = existing.availableQuantity;
        const updated = await this.warehouseRepository.updateWarehouseInventory(existing.id, dto);
        await this.auditService.log({
    ...
    async transferStock(dto, userId) {
        if (dto.fromWarehouseId === dto.toWarehouseId)
            throw new exceptions_1.ValidationException('Cannot transfer to the same warehouse', 'WAREHOUSE_005');
        const fromWh = await this.warehouseRepository.findById(dto.fromWarehouseId);
        if (!fromWh || fromWh.deletedAt)
            throw new exceptions_1.BusinessException('Source warehouse not found', 'WAREHOUSE_001');
        const toWh = await this.warehouseRepository.findById(dto.toWarehouseId);
        if (!toWh || toWh.deletedAt)
            throw new exceptions_1.BusinessException('Destination warehouse not found', 'WAREHOUSE_001');
        const fromInv = await this.warehouseRepository.findWarehouseInventory(dto.fromWarehouseId, dto.variantId);
        if (!fromInv)
            throw new exceptions_1.BusinessException('No inventory found in source warehouse', 'WAREHOUSE_006');
        const available = fromInv.availableQuantity - fromInv.reservedQuantity;
        if (dto.quantity > available)
            throw new exceptions_1.ValidationException('Insufficient stock for transfer', 'WAREHOUSE_007');
        ...
        const toInv = await this.warehouseRepository.findWarehouseInventory(dto.toWarehouseId, dto.variantId);
        if (toInv) {
