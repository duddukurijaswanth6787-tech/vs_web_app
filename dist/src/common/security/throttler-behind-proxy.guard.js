"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ThrottlerBehindProxyGuard = void 0;
const common_1 = require("@nestjs/common");
const throttler_1 = require("@nestjs/throttler");
const core_1 = require("@nestjs/core");
const security_utils_1 = require("./security.utils");
const security_event_service_1 = require("./security-event.service");
let ThrottlerBehindProxyGuard = class ThrottlerBehindProxyGuard extends throttler_1.ThrottlerGuard {
    securityEventService;
    options;
    storageService;
    reflector;
    constructor(securityEventService, options, storageService, reflector) {
        super(options, storageService, reflector);
        this.securityEventService = securityEventService;
        this.options = options;
        this.storageService = storageService;
        this.reflector = reflector;
    }
    async getTracker(req) {
        return security_utils_1.SecurityUtils.getClientIp(req);
    }
    async throwThrottlingException(context, throttlerLimitDetail) {
        const req = context.switchToHttp().getRequest();
        this.securityEventService.log({
            type: security_event_service_1.SecurityEventType.RATE_LIMIT_VIOLATION,
            ip: security_utils_1.SecurityUtils.getClientIp(req),
            path: req.url,
            method: req.method,
            correlationId: req.headers['x-correlation-id'] || '',
            userAgent: security_utils_1.SecurityUtils.getUserAgent(req),
        });
        return super.throwThrottlingException(context, throttlerLimitDetail);
    }
};
exports.ThrottlerBehindProxyGuard = ThrottlerBehindProxyGuard;
exports.ThrottlerBehindProxyGuard = ThrottlerBehindProxyGuard = __decorate([
    (0, common_1.Injectable)(),
    __param(1, (0, throttler_1.InjectThrottlerOptions)()),
    __param(2, (0, throttler_1.InjectThrottlerStorage)()),
    __metadata("design:paramtypes", [security_event_service_1.SecurityEventService, Object, Object, core_1.Reflector])
], ThrottlerBehindProxyGuard);
//# sourceMappingURL=throttler-behind-proxy.guard.js.map