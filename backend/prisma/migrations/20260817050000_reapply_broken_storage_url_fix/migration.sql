-- Re-applies the same localhost:4000 -> real backend origin fix as
-- migration 20260815190000. That one fixed the data as it stood at the
-- time, but at least one browser tab still had the pre-fix values sitting
-- in its own client-side cache (from before AWS_S3_PUBLIC_URL was
-- corrected) and re-saved one of them, re-introducing a broken URL on a
-- banner. The write paths themselves are now guarded server-side
-- (StorageService.sanitizeUrl, wired into banners/categories/product
-- media/library media) so this shouldn't recur, but this migration
-- catches whatever is broken right now regardless of cause.
--
...
UPDATE "banners" SET
  ...
  "mobileImageUrl" = replace("mobileImageUrl", 'http://localhost:4000/api/v1/storage', 'https://vsss-production.up.railway.app/api/v1/storage')
WHERE "imageUrl" LIKE 'http://localhost:4000%'
   OR "mobileImageUrl" LIKE 'http://localhost:4000%';
