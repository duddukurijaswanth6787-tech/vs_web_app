...
export class AnalyticsController {
  ...
  @Get('omnichannel')
  @ApiOperation({
    summary: 'Get Omnichannel Overview Analytics (POS vs Online)',
  })
  async getOmnichannel(@Query('period') period?: AnalyticsPeriod) {
    const data = await this.analyticsService.getOmnichannelOverview(
      period || 'monthly',
    );
    return ResponseBuilder.success(
      data,
      'Omnichannel analytics fetched successfully',
    );
  }
  ...
  async getOfflinePos(@Query('period') period?: AnalyticsPeriod) {
    const data = await this.analyticsService.getOfflinePosAnalytics(
      period || 'monthly',
    );
  ...
  async getOnlineSales(@Query('period') period?: AnalyticsPeriod) {
    const data = await this.analyticsService.getOnlineSalesAnalytics(
      period || 'monthly',
    );
    return ResponseBuilder.success(
      data,
      'Online sales analytics fetched successfully',
    );
  }

  @Get('inventory-velocity')
  @ApiOperation({
    summary: 'Get Fast-Moving vs Slow-Moving Inventory Velocity',
  })
  async getInventoryVelocity() {
    const data = await this.analyticsService.getInventoryVelocityAnalytics();
    return ResponseBuilder.success(
      data,
      'Inventory velocity analytics fetched successfully',
    );
  }
}
